(function() {
  'use strict';
  var HTML, SUBSIDIARY, TI, log, set_getter, state, µ;
  ...
  <animateTransform attributeName="transform" type="rotate" from="0 67 67" to="360 67 67" dur="8s" repeatCount="indefinite"/>
    ...
    connectedCallback() {
      ...
      log('^243^', µ.DOM.on(this, 'E', function() {
        return log('^webc@325-1^', "received `E`");
      }));
      return null;
    }
  ...
  div {
    ...
    outline:  1px solid red; }
      ...
      connectedCallback() {
        ...
        log('^243^', µ.DOM);
        log('^243^', µ.DOM.on(this, 'E', function() {
          return log('^webc@325-4^', "received `E`");
        }));
        return null;
      }
    ...
    //---------------------------------------------------------------------------------------------------------
    My_counter.$ = {
      on_counter_increment: function() {
        var counter;
        log('^webc@325-2^', "counter-increment");
        log('^webc@325-3^', counter = parseInt(this._.$div.textContent, 10));
        if (counter === 9) {
